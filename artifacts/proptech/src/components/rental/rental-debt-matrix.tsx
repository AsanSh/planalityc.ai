...
import { MATRIX_TH } from "@/components/matrix-table-frame";
...
export function RentalDebtMatrix({
	...
	return (
		<div
			className="am-table-wrap rounded-[18px] overflow-auto"
			style={{ maxHeight }}
		>
			<table
			...
			>
				<thead>
					<tr>
						<th
							className="border border-slate-800 bg-slate-950 text-center text-white/70 font-bold py-2 px-2 select-none sticky top-0 left-0 z-30 text-[11px] w-10 shadow-none"
						>
							#
						</th>
						{allCols.map((col) => {
							...
							return (
								<th
									...
									className={cn(
										MATRIX_TH,
										"border border-slate-800 py-2 px-2 text-[10px] sticky top-0 z-20 shadow-none relative",
										col.align === "right" && "text-right",
										col.align === "center" && "text-center",
										sortable && "cursor-pointer hover:bg-slate-800",
				...
				<tbody>
					{isLoading ? (
						...
						rows.map((row, i) => {
							...
							return (
								<tr key={row.key} className={cn(bg, "transition-colors hover:bg-cyan-50/70")}>
									<td
